...
STU* Creat(int n, int m) {
	...
	for (i = 1; i < n + 1; i++)
	{
		p1 = (STU*)malloc(LEN);
		printf("\t\t\t");
		scanf_s("%ld", &p1->num,sizeof(p1->num));
		printf("\t\t\t");
		scanf_s("%s", p1->name,sizeof(p1->name));
		for (j = 0; j < m; j++)
		{
			printf("\t\t\t");
			scanf_s("%f", &p1->score[j]);
		}
		p1->next = NULL;
		if (i == 1)
		{
...
void  WritetoFile(STU * head, int n, int m)
{
	...
	printf("\n\n\n");
	printf("\t\t\t******************************************************************************\n");
	if ((err = fopen_s(&fp,"student.txt", "w")) == NULL)
	{
...
STU* ReadfromFile(STU * head, int* n, int* m)
{
	...
	int i, j;
	errno_t err;  //判断此文件流是否存在 存在返回1
	if ((fp = fopen_s(&fp,"student.txt", "r")) == NULL)
	{
		printf("\t\t\tFail to open student.txt\n");
		return NULL;
	}
	...
	for (i = 0; i < *n; i++)
	{
		fscanf_s(fp, "%12ld", &p->num);
		fscanf_s(fp, "%12s", &p->name);
		for (j = 0; j < *m; j++)
		{
			fscanf_s(fp, "%12f", &p->score[j]);
		}
		fscanf_s(fp, "%12f%12f", &p->sum, &p->aver);
